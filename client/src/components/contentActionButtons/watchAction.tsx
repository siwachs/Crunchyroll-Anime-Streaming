...
const WatchAction: React.FC<{
  ...
  tabIndex?: number;
  className?: string;
}> = ({ watchActionhref, watchActionText, tabIndex = 0, className = "" }) => {
  return (
    <Link
      tabIndex={tabIndex}
      href={watchActionhref}
      prefetch={false}
      className={`action-button flex w-full min-w-[7.5rem] flex-1 bg-[var(--app-background-crunchyroll-orange)] px-4 text-black hover:bg-[var(--app-hover-crunchyroll-orange)] hover:text-[var(--app-background-secondary)] focus-visible:bg-[var(--app-hover-crunchyroll-orange)] focus-visible:text-[var(--app-background-secondary)] sm:w-auto sm:flex-[0_0_auto] ${className}`}
    >
